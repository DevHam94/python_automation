        ...
        for row in ws.iter_rows(min_row=2):
            if row[0].value != None:
                with open(self.template_filename, encoding='utf-8') as f:
                    ...
                    temp1 = temp1.replace('%받는분%', row[2].replace)
                    temp1 = temp1.replace('%매니저_이름%', self.manager_name)
                    print(self.send_email(html_msg=temp1,
...
if __name__ == '__main__':
    es = EmailSender('fc.krkim@gmail.com', os.getenv('MY_GMAIL_PASSWORD'), manager_name='김미령', path='data220920/')
